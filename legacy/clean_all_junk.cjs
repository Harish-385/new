...
for (const dept of Object.keys(data)) {
  for (const subpage of Object.keys(data[dept])) {
    const content = data[dept][subpage].content;
    if (!content) continue;
    ...
    if (junkIndex !== -1) {
      console.log(`Cleaning up '${dept}/${subpage}'. Removing ${content.length - junkIndex} items.`);
      data[dept][subpage].content = content.slice(0, junkIndex);
      modifiedCount++;
    }
  }
}
...
if (modifiedCount > 0) {
  ...
  console.log(`Successfully updated ${modifiedCount} subpages.`);
} else {
