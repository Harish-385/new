...
export const MechDept: React.FC<DeptProps> = () => {
  ...
  const isFacultyProfilePage = activeSubpage.toLowerCase().includes('faculty profile')  || activeSubpage.toLowerCase().includes('supporting staff profile') || activeSubpage.toLowerCase().includes('alumni interaction') || activeSubpage.toLowerCase().includes('adjunct professor') || activeSubpage.toLowerCase().includes('honorary professor')

  const facultyMembers = isFacultyProfilePage ? (() => {
    if (subpageData?.faculty && subpageData.faculty.length > 0) {
      return subpageData.faculty
    }
  ...
  const textItems = isFacultyProfilePage
    ? filteredContentItems.filter((item) => {
        ...
        if (item.type === 'image' && item.src && facultyMembers.some((fac: any) => fac.image === item.src)) return false
        return true
      })
    : filteredContentItems.filter((item) => {
