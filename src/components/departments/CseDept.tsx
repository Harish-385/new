...
export const CseDept: React.FC<DeptProps> = () => {
  ...
  const isFacultyProfilePage = activeSubpage.toLowerCase().includes('faculty profile') || activeSubpage.toLowerCase().includes('supporting staff profile') 
  ...
  const pdfAttachmentCards = (() => {
    ...
    const getHref = (row: any[]): string | null => {
      const linkCell = [...row].reverse().find((cell) => cell && typeof cell === 'object' && cell.href)
    ...
    return tableRows
      ...
      .map((row, index) => {
        ...
        const title = row.slice(1, -1).map(getStr).join(' ').trim() || getStr(row[1]) || 'Open PDF'
        return {
          year,
          title,
          href: getHref(row) || pageUrl,
