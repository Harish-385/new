...
export const CivilDept: React.FC<DeptProps> = () => {
  ...
  const filteredContentItems = rawContentItems.filter((item) => {
    if (item.type === 'image') {
      // HOD photos (e.g. 181_Dharmar.jpg) get caught by the shared numeric-filename
      // blocklist in deptHelpers; always keep them since alt text marks them explicitly.
      if (/^hod\b/i.test(item.alt || '')) return true
